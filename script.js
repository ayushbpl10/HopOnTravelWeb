...
async function submitWebBooking() {
  ...
  try {
    ...
    let waNum = (vendorWhatsApp || '').replace(/\D/g, '');
    if (!waNum || waNum.length < 10) {
      // Fallback to ATGL email support if vendor number is missing
      const mailMsg = encodeURIComponent(`Booking ID: ${bookingId}\nName: ${name}\nPackage: ${packageName} x${seats}\nTotal: ₹${totalPrice}\n\nPlease confirm my booking!`);
      const waBtn = document.getElementById('conf-whatsapp');
      waBtn.href = `mailto:abtohghoomle@gmail.com?subject=Booking ${bookingId}&body=${mailMsg}`;
      waBtn.innerHTML = `✉️ Email Support`;
    } else {
